...
> 📋 **Полный перечень проверок.** Подробный план тестирования с примерами
> сценариев для каждого требования лабораторной работы вынесен в отдельный
> документ [`docs/TESTING_GUIDE.md`](docs/TESTING_GUIDE.md). В нём перечислены
> все группы тестов (конструкторы, управление памятью, арифметика,
> вычислительные методы и ключевые математические свойства), а также описаны
> команды для их запуска через GoogleTest.

> 🚀 **Публикация изменений на GitHub.** Пошаговый алгоритм подготовки коммита,
> работы с подмодулями и отправки правок в удалённый репозиторий описан в
> документе [`docs/GITHUB_PUSH.md`](docs/GITHUB_PUSH.md). Используйте его,
> когда будете финализировать работу и делиться результатами через GitHub.

## Быстрая автоматическая проверка в этом репозитории
Для отладки и демонстрации готовности библиотеки в офлайн-среде предусмотрен
отдельный набор тестов, собранный вместе с упрощённой реализацией GoogleTest.
Он полностью покрывает требования лабораторной работы и запускается
непосредственно из этого репозитория.

```bash
cmake -S . -B build-tests -DLINALG_BUILD_TESTS=ON
cmake --build build-tests
ctest --test-dir build-tests --output-on-failure
```

- Опция `LINALG_BUILD_TESTS=ON` включает цель `matrix_full_tests` и собирает
  лёгкий раннер GoogleTest, расположенный в `third_party/googletest`.
- После сборки `ctest` выполнит весь набор проверок (конструкторы, управление
  памятью, арифметика, вычислительные функции и свойства из методички).
- Логи запуска содержат отчёт по каждому тестовому кейсу, что удобно для
  быстрой диагностики и последующего переноса сценариев в основной
...
### Быстрый запуск демонстрации в CLion
В репозитории уже есть готовый пример `examples/buffer_management_demo.cpp`, который
собирается по запросу. Это полезно для второго шага лабораторной, когда требуется
проверить изменение размеров и вместимости матрицы.

1. Откройте настройки CMake в CLion и добавьте опцию `-DLINALG_BUILD_EXAMPLES=ON`
   (или выполните `cmake -S . -B build -DLINALG_BUILD_EXAMPLES=ON` вручную).
2. После пересборки появится цель `linalg_buffer_demo`. Запустите именно её, а не
   библиотеку `linalg` — запуск статической библиотеки приводит к ошибке
   `CreateProcess error=193`, потому что библиотека не является исполняемым файлом.
3. В окне вывода вы увидите последовательность состояний матрицы и сможете убедиться,
   что операции выполняются корректно.

Если хочется изучить код примера или собрать его вручную, ниже приведён исходник
утилиты:
